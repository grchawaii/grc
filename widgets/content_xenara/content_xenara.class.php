<?php
    ...
    class content_xenara extends WidgetHandler {
        ...
        function proc($args) {
            ...
            if(Context::get('order_target')) $args->order_target = Context::get('order_target');
            
            // Sort order
            if($args->order_type!='desc') $args->order_type = 'asc';
//            if(Context::get('order_type')) $args->order_type = Context::get('order_type'); //게시판과 사용시 중복문제가 발생
            ...
            // 추가옵션 미디어설정
            if(!$args->media_width) $args->media_width = 560;
            if(!$args->media_height) $args->media_height = 315;
            ...
            // Get module information after listing module_srls if the module is not RSS
                ...
                if(!$args->module_srls){
                    ...
                    $output = executeQueryArray('widgets.content_xenara.getMids', $obj);
                    if($output->data) {
        ...
        function _getCommentItems($args) {
            ...
            foreach($output as $key => $oComment) {
                ...
                $content_item = new content_xenaraItem($browser_title);
        ...
        function _getDocumentItems($args){
            ...
            // Get categories
            $obj->module_srl = $args->module_srl;
            $output = executeQueryArray('widgets.content_xenara.getCategories',$obj);
...
//            $output = $oDocumentModel->getDocumentList($obj); //공지글제외 추출
//            $output = $oDocumentModel->getNoticeList($obj); //공지글만 추출
            $output = executeQueryArray('widgets.content_xenara.getNewestDocuments', $obj);
            ...
            if(count($output->data)) {
                ...
                for($i=0,$c=count($document_srls);$i<$c;$i++) {
                    ...
                    $content_item = new content_xenaraItem( $args->module_srls_info[$module_srl]->browser_title );
                    ...
                    $content_item->add('is_notice', $oDocument->get('is_notice'));
       ...
       /**
        ...
        function _getRssItems($args){
            ...
            if($xml_doc->rss) {
                ...
                foreach ($items as $key => $value) {
                    ...
                    $content_item = new content_xenaraItem($rss->title);
        ...
        function _compile($args,$content_items){
            ...
            // 추가옵션 지도설정
            $widget_info->map_provider = $args->map_provider;
            $widget_info->naver_map_apikey = $args->naver_map_apikey;
            $widget_info->map_addr_eid = $args->map_addr_eid;
            $widget_info->map_width = $args->map_width;
            $widget_info->map_height = $args->map_height;
            // 샵소개 게시판 v1.0 전용
            $widget_info->shopintro_extra = $args->shopintro_extra;
            ...
            $widget_info->duration_new2 = $args->duration_new;
            ...
            if($args->tab_type != 'none' && $args->tab_type) {
                ...
                foreach($args->mid_lists as $module_srl => $mid){
                    ...
                    //추가사항 - 시작
                    $tab_item->module_srl = $module_srl;
                    $tab_item->mid = $mid;
                    //추가사항 - 끝
                ...
                //추가사항 - 시작(선택한 모듈정보를 조건없이 출력)
                $tab2 = array();
                $tab_module_srls = explode(',', $args->module_srls);
                $oModuleModel = &getModel('module');
                $oMenuAdminModel = &getAdminModel('menu');
                foreach($tab_module_srls as $key => $module_srl){
                    unset($tab_item2);
                    $tabModuleInfo = $oModuleModel->getModuleInfoByModuleSrl($module_srl);
                    $menuItemInfo = $oMenuAdminModel->getMenuItemInfo($tabModuleInfo->menu_srl);
                    $tab_item2->title = $tabModuleInfo->browser_title;
                    $tab_item2->content_items = $content_items[$module_srl];
                    $tab_item2->url = getUrl('','mid',$tabModuleInfo->mid);
                    $tab_item2->module_srl = $module_srl;
                    $tab_item2->mid = $mid;
                    $tab_item2->menu_srl = $menuItemInfo->menu_srl;
                    $tab_item2->menu_name = $menuItemInfo->name;
                    $tab2[] = $tab_item2;
                }
                $widget_info->tab2 = $tab2;
                //추가사항 - 끝
    ...
    class content_xenaraItem extends BaseObject {
        ...
        function content_xenaraItem($browser_title=''){
