...
<?php if($__Context->XE_VALIDATOR_MESSAGE && $__Context->XE_VALIDATOR_ID == 'modules/poll/tpl/poll_list/1'){ ?><div class="message <?php echo escape($__Context->XE_VALIDATOR_MESSAGE_TYPE, false) ?>">
...
echo escape(getUrl(''), false) ?>?document_srl=<?php echo escape($__Context->val->document_srl, false);
};
if($__Context->val->comment_srl){ ?>#comment_<?php echo escape($__Context->val->comment_srl, false);
} ?>" target="_blank"><?php echo escape($__Context->val->title, false) ?></a></td>
				<td class="nowr"><?php if($__Context->val->checkcount == 1){;
echo $__Context->lang->single_check;
}else{;
echo $__Context->lang->multi_check;
} ?></td>
				<td class="nowr"><?php echo escape($__Context->val->poll_count, false) ?></td>
				<td class="nowr"><a href="#popup_menu_area" class="member_<?php echo escape($__Context->val->member_srl, false) ?>"><?php echo escape($__Context->val->nick_name, false) ?></a></td>
				<td class="nowr"><?php echo escape(zdate($__Context->val->poll_regdate,"Y-m-d H:i"), false) ?></td>
				<td class="nowr"><?php echo escape(zdate($__Context->val->poll_stop_date,"Y-m-d"), false) ?></td>
				<td>
					<input type="checkbox" name="cart[]" value="<?php echo escape($__Context->val->poll_index_srl, false) ?>" />
				</td>
			</tr><?php } ?>
...
<form action="./" method="get" class="search center x_input-append" style="clear:both;padding:10px 0 0 0;margin:0" ><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
	...
	<select name="search_target" title="<?php echo $__Context->lang->search_target ?>" style="margin-right:4px">
		<?php if($__Context->lang->search_poll_target_list&&count($__Context->lang->search_poll_target_list))foreach($__Context->lang->search_poll_target_list as $__Context->key=>$__Context->val){ ?><option value="<?php echo escape($__Context->key, false) ?>"<?php if($__Context->search_target==$__Context->key){ ?> selected="selected"<?php } ?>><?php echo escape($__Context->val, false) ?></option><?php } ?>
	</select>
