<?php if(!defined("__XE__"))exit;?><div class="board_read">
	...
	<div class="read_header">
		...
		<p class="meta">
			<?php if($__Context->module_info->display_author!='N' && !$__Context->oDocument->getMemberSrl() && $__Context->oDocument->isExistsHomepage()){ ?><a href="<?php echo $__Context->oDocument->getHomepageUrl() ?>" target="_blank"  rel="nofollow noopener" class="author"><?php echo $__Context->oDocument->getNickName() ?></a><?php } ?>
...
echo $__Context->file->download_url ?>" rel="nofollow"><?php echo $__Context->file->source_filename ?> <span class="fileSize">[File Size:<?php echo FileHandler::filesize($__Context->file->file_size) ?>/Download:<?php echo number_format($__Context->file->download_count) ?>]</span></a></li><?php } ?>
		...
		<div class="btnArea"> 
			<?php if($__Context->oDocument->isEditable()){ ?><a class="btn" href="<?php echo getUrl('act','dispBoardWrite','document_srl',$__Context->oDocument->document_srl,'comment_srl','') ?>" rel="nofollow"><?php echo $__Context->lang->cmd_modify ?></a><?php } ?>
			<?php if($__Context->oDocument->isEditable()){ ?><a class="btn" href="<?php echo getUrl('act','dispBoardDelete','document_srl',$__Context->oDocument->document_srl,'comment_srl','') ?>" rel="nofollow"><?php echo $__Context->lang->cmd_delete ?></a><?php } ?>
			<span class="etc">
