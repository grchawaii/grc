...
${'var_idx45_argument'} = new ConditionArgument('var_idx', $args->var_idx, 'equal');
${'var_idx45_argument'}->checkFilter('number');
${'var_idx45_argument'}->createConditionValue();
if(!${'var_idx45_argument'}->isValid()) return ${'var_idx45_argument'}->getErrorMessage();
} else
${'var_idx45_argument'} = NULL;if(${'var_idx45_argument'} !== null) ${'var_idx45_argument'}->setColumnType('number');

${'sort_index46_argument'} = new Argument('sort_index', $args->{'sort_index'});
${'sort_index46_argument'}->ensureDefaultValue('var_idx');
...
,new ConditionWithArgument('`var_idx`',$var_idx45_argument,"equal", 'and')))
));
$query->setGroups(array());
$query->setOrder(array(
new OrderByColumn(${'sort_index46_argument'}, "asc")
));
$query->setLimit();
return $query; ?>